...
//---------------------------------------------------------------------------------------------------------------------------------
    this.gatherCompletions = function(editor, callback) {
       ...
       //alert(prefix);
        var item = prefix.split('.');
        this.keywordlist = item;
        prefix = item[item.length - 1];
        //alert([item[0],prefix]);
        //if (prefix == "") return false;
        //alert([item, prefix]);
        //prefix=prefix.split('.')[prefix.length - 1];
        //alert(prefix);
        //alert(this.detach);
        this.base = session.doc.createAnchor(pos.row, pos.column - prefix.length);
        //this.base = session.doc.createAnchor(pos.row,5);
    ...
    this.updateCompletions = function(keepPopupPosition) {
        if (keepPopupPosition && this.base && this.completions) {
            ...
            if (prefix == this.completions.filterText)
                return;
            this.completions.setFilter(prefix, this.keywordlist);
...
var FilteredList = function(array, filterText) {
    ...
    (function () {
        //----------------------------------------------------
    this.setFilter = function (str, keywordlist) {
        this.keywordlist = keywordlist;
        //alert([str,this.keywordlist])
...
ace.define("ace/autocomplete/text_completer",["require","exports","module","ace/range"], function(require, exports, module) {
    ...
    function wordDistance(doc, pos) {
        ...
        var k = doc.getValue();//获取当前正文中数据
        k = "";
        var words = k.split(splitRegex);//分析出当前正文中包含的文键词
...
var keyWordCompleter = {
    getCompletions: function (editor, session, pos, prefix, callback,keywordlist) {
        if (session.$mode.completer) {
            return session.$mode.completer.getCompletions(editor, session, pos, prefix, callback);
        }
        var state = editor.session.getState(pos.row);
        var completions = session.$mode.getCompletions(state, session, pos, prefix, keywordlist);
...
var completers = [textCompleter, keyWordCompleter];
...
var doLiveAutocomplete = function(e) {
    ...
    if (e.command.name === "backspace") {
        ...
        var prefix = util.getCompletionPrefix(editor);
        var r = prefix.substring(prefix.length-1);

...
(function() {
    //-----------------------------------根据情况加载不同关键词组------------------------------------
    this.getCompletions = function(state, session, pos, prefix,keywordlist) {
        //razor-short-start  短语关键词
        //razor-block-start  代码块
        ...
        if (state.lastIndexOf("razor-short-start") != -1) {
            return this.getShortStartCompletions(state, session, pos, prefix, keywordlist);
        }
        
        if (state.lastIndexOf("razor-block-start") != -1) {
            return this.getKeywordCompletions(state, session, pos, prefix, keywordlist);
        }

        
    };

    this.getShortStartCompletions = function (state, session, pos, prefix, keywordlist) {
    ...
    this.getKeywordCompletions = function (state, session, pos, prefix, keywordlist) {
        var count = ace.shortKeyword.length;
        var temp = [];
        var pkey = keywordlist[keywordlist.length - 2];
        for (var i = 0; i < count; i++) {
            if (ace.shortKeyword[i].p==pkey)temp.push(ace.shortKeyword[i]);
        }
        return temp;
...
(function() {
    this.getCompletions = function(state, session, pos, prefix,keywordlist) {
        var razorToken = this.$completer.getCompletions(state, session, pos, prefix, keywordlist);
        var htmlToken = this.$htmlCompleter.getCompletions(state, session, pos, prefix, keywordlist);

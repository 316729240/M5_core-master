...
$M.system.dataManage = function (S) {
    ...
    extensionMenu = $M.loadInterface(_p, S.dataTypeId, function (name) {
        $M.app.call(name, { moduleId: item._moduleId, classId: item._classId, type: item._type, searchField: item._searchField, keyword: item._keyword, reload: reload });
    });
    ...
    if ($.dataManage[S.dataTypeId]) {
        ...
        var searchMenu = $(document.body).addControl({ xtype: "Menu", onItemClicked:
            function (sender, e) {
                ...
                searchInput.setShow(e.attr("type"));
            }, items: [{ text: "id", value: "id",type:0 }, { text: "标题", value: "title",type:1 }, { text: "发布人", value: "userId" ,type:2}]
        });
        var attrIndex = -1;

        $M.comm("fieldInfo", { id: item._dataTypeId }, function (json) {
            var index = 0;
            var searchitem = [];
            for (var i = 0; i < json.length; i++) {
                if (json[i].name.indexOf("u_") > -1 || json[i].name=="title") {
                    var type = 0;
                    if (json[i].type == "String") type = 0;
                    else if (json[i].type == "Double" || json[i].type == "Number") type = 1;
                    else if (json[i].type == "DateTime") type = 2;
                    searchitem[searchitem.length] = { text: json[i].text, value: json[i].name, type: type };
                }
            }
            searchMenu.attr("items", searchitem);
        });
        ...
        var toolBar = item.addControl({
            ...
            items: [
                ...
                [{ xtype: "InputGroup", name: "searchGroup", style: { width: "400px" }, items: [
                    { xtype: "Button", text: "搜索'标题'", menu: searchMenu, name: "searchF" },
                    { xtype: "SearchInput", text: "", name: "searchInput" },

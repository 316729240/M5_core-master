...
namespace Microshaoft.Text
{
        ...
        internal static string[] nicename = new string[]
   {
    ...
    , "ISO 2022CN", "UTF-8", "Unicode", "ASCII", "OTHER"
        ...
        public virtual string GetEncodingName(byte[] rawtext)
        {
            int max = rawtext.Length > 1000 ? 1000 : rawtext.Length;
            sbyte[] target = new sbyte[max];



            for (int i = 0; i < max; i++)
                target[i] = (sbyte)rawtext[i];
            return (GetEncodingName(target));
        }
        #endregion
